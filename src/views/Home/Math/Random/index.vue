...
<div class="msg">
  ...
  <div class="add-main">
    <div class="main-area">
        {{n1}}的近似数为 <input type = "text" placeholder="请输入结果" class = "input" v-model = "value" @keyup.enter = "getresult"/>
   </div>
...
methods:{
    render_num(){
    this.n1 = random(0,1000)
    this.nums.push({n1:this.n1,flag:false})
    },
    prev(){
    if(this.count >= 1){
    this.count --;
    ...
    next(){
        ...
        if((+this.value - this.n1) >= -10 && (+this.value - this.n1) <= 10 && !obj.flag){
            this.result += 10
            obj.flag = true
        }
         ...
         persons.forEach(item=>{
            if(item.status == true){
                ...
                if(this.result == 0){
                 ...
                 item.math.unshift({random:this.result,time,})
            }
         })
         saveInfo(persons)
        }
